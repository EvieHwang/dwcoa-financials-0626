...
def test_copy_into_nonempty_target_requires_overwrite(admin_client, app_env):
    db = app_env["db_path"]
    _seed_two_lines(db, 2024)
    # Target already has a line.
    existing = category_id(db, "Fire Alarm")
    insert_budget(db, year=2025, category_id=existing, annual_amount=330000)
    ...
    assert count_budgets(db, 2025) == 1
    assert get_budget_row(db, year=2025, category_id=existing)["annual_amount"] == 330000


def test_copy_overwrite_replaces_target(admin_client, app_env):
    db = app_env["db_path"]
    a, b = _seed_two_lines(db, 2024)
    # A different pre-existing target line that should be gone after replace.
    stale = category_id(db, "Fire Alarm")
    insert_budget(db, year=2025, category_id=stale, annual_amount=330000)

    r = admin_client.post("/api/budgets/copy", json={
        "from_year": 2024, "to_year": 2025, "overwrite": True,
    })
    assert r.status_code == 200
    # 2025 now matches 2024's set: the two source lines, and the stale line gone.
    assert count_budgets(db, 2025) == 2
    assert get_budget_row(db, year=2025, category_id=stale) is None
    assert get_budget_row(db, year=2025, category_id=a)["annual_amount"] == 450000
    assert get_budget_row(db, year=2025, category_id=b)["annual_amount"] == 600000


# --- R4: lock / unlock ------------------------------------------------------

def test_lock_then_read_reflects_state(admin_client, app_env):
    r = admin_client.post("/api/budgets/lock", json={"year": 2025, "locked": True})
    assert r.status_code == 200
    assert r.json()["locked"] is True
    assert r.json()["locked_at"]  # a timestamp is set
    body = admin_client.get("/api/budgets?year=2025").json()
...
def test_unlock_restores_writeability(admin_client, app_env):
    ...
    set_lock(db, year=2025, locked=True)
    # Locked -> write refused (R5).
    assert admin_client.post("/api/budgets", json={
        "year": 2025, "category_id": cid, "annual_amount": 1000,
    }).status_code == 403
    # Unlock, then the same write succeeds.
    admin_client.post("/api/budgets/lock", json={"year": 2025, "locked": False})
    assert admin_client.post("/api/budgets", json={
        "year": 2025, "category_id": cid, "annual_amount": 1000,
    }).status_code == 200
...
def test_locked_upsert_refused_and_unchanged(admin_client, app_env):
    ...
    insert_budget(db, year=2025, category_id=cid, annual_amount=450000)
    set_lock(db, year=2025, locked=True)
    r = admin_client.post("/api/budgets", json={
        "year": 2025, "category_id": cid, "annual_amount": 999999,
    })
    assert r.status_code == 403
    assert get_budget_row(db, year=2025, category_id=cid)["annual_amount"] == 450000
...
def test_locked_copy_into_refused_and_unchanged(admin_client, app_env):
    ...
    assert r.status_code == 403
    assert count_budgets(db, 2025) == 0
...
def test_locked_delete_refused_and_unchanged(admin_client, app_env):
    ...
    insert_budget(db, year=2025, category_id=cid, annual_amount=450000)
    set_lock(db, year=2025, locked=True)
    r = admin_client.delete(f"/api/budgets?year=2025&category_id={cid}")
    assert r.status_code == 403
    assert get_budget_row(db, year=2025, category_id=cid) is not None
